...
type PracticeRepository interface {
	...
	GetRandomQuestionID(ctx context.Context, topicID *uuid.UUID, level *string) (uuid.UUID, error)
	...
	GetQuestionContent(ctx context.Context, questionID uuid.UUID) (string, string, string, string, error) // returns content, topic, level, correctAnswer
...
type AIService interface {
	EvaluateAnswer(ctx context.Context, question, userAnswer, topic, level, language string) (int, string, []string, string, error) // score, feedback, suggestions, improvedAnswer
}

type PracticeService interface {
	StartSession(ctx context.Context, userID uuid.UUID, topicID *uuid.UUID, level *string) (*domain.PracticeSession, uuid.UUID, error)
	...
	GetSession(ctx context.Context, id uuid.UUID) (*domain.PracticeSession, error)
	GetQuestion(ctx context.Context, questionID uuid.UUID) (string, string, string, string, error)
	CreateQuestion(ctx context.Context, content, topic, level, correctAnswer string) (*domain.Question, error)
	GetTopicIDByName(ctx context.Context, name string) (uuid.UUID, error)
}
